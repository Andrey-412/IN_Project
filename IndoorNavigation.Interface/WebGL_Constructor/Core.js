...
IndoorNavigation.Core = function( container , initHelpers )
{
	...
	Array.prototype.findPrototype = function (obj) {
	    var i = this.length;
	    while (i--) {
	        if (this[i].__proto__ == obj.__proto__) {
	            this[i].Dispose();	            
	            break;
	        }
	    }
...
IndoorNavigation.NetworkModule.Door = function ( data, object )
{
	...
	function LoadModel() {
	    ...
	    loader.load(
            // resource URL
            '/WebGL_Constructor/Objects/MARKER1.xml',
...
{
    this.CreatePathTest = function (markersPositions, useConverter) {
        if (useConverter) {
            var pos = [];
            var data = {
                "TranslateX": -10,
            ...
            for (var i = 0; i < markersPositions.length; i++) {
                var positionVector = new THREE.Vector3(markersPositions[i].X + data.TranslateX,
                    markersPositions[i].Y + data.TranslateY,
                    markersPositions[i].Z + data.TranslateZ).multiplyScalar(100);
                pos.push(positionVector);
            }
            var path = new IndoorNavigation.API.Path().CreatePath(pos);
        }
        else
            var path = new IndoorNavigation.API.Path().CreatePath(markersPositions);
    }
...
IndoorNavigation.API.Path = function () {
    ...
    this.CreatePath = function (markersPositions) {
        ...
        IndoorNavigation.Core.APIfunctions.findPrototype(this);

        var texturePath = '/WebGL_Constructor/Assets/disc.png';
        var scope = this;
        scope.enabled = true;
        ...
        function Init() {
            ...
            scope.particleCube = new THREE.ParticleSystem(scope.particleGeometry, shaderMaterial);

            scope.particleCube.position.set(0, 15, 0);

            scope.particleCube.dynamic = true;
            ...
            scope.particleCube.sortParticles = true;
            IndoorNavigation.mainScene.add(scope.particleCube);
        }

        this.Dispose = function(index) {
            IndoorNavigation.Core.APIfunctions.splice(index, 1);
            IndoorNavigation.mainScene.remove(scope.particleCube);
        }
...
IndoorNavigation.WiresModule = function (markersPositions) {
    ...
    function Init() {
        ...
        var discTexture = THREE.ImageUtils.loadTexture('/WebGL_Constructor/Assets/disc.png');
